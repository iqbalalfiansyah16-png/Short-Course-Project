...
// Dual export pattern: support Node.js dan Browser
(function (root, factory) {
    if (typeof module !== "undefined" && module.exports) {
      // Node.js environment
      const EnhancedTask = require("../models/EnhancedTask");
      module.exports = factory(EnhancedTask);
    } else {
      // Browser environment - cek apakah sudah di-load
      if (root.TaskRepository) {
        return;
      }
      root.TaskRepository = factory(root.EnhancedTask);
    }
  })(typeof self !== "undefined" ? self : this, function (EnhancedTask) {
    class TaskRepository {
      ...
      findDueSoon(days = 3) {
        return this.findAll().filter((task) => {
          ...
          return (
            daysUntilDue !== null && daysUntilDue <= days && daysUntilDue >= 0
          );
        });
      }
      ...
      getStats(userId = null) {
        ...
        // Count by category - gunakan dynamic categories dari model
      ...
      getMostUsedCategories(userId = null, limit = 5) {
        ...
        return Object.entries(stats)
          ...
          .map(([category, data]) => ({
            category,
            count: data.total,
            displayName: EnhancedTask.prototype.getCategoryDisplayName.call({
              _category: category,
            }),
          }));
      }
